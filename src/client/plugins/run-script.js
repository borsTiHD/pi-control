export default ({ app }, inject) => {
     ...
     * @param       {boolean}       silent  -> Console.log response will not be displayed
     * @returns     {data|error}            -> Returns script output data
     */
    const runScript = async(script, args = [], silent) => {
        const url = '/scripts/execute' // API Endpoint
        const data = await app.$axios.post(url, null, { params: { script, args } })
            .then((res) => {
                const data = res.data
                if (!silent) { console.log(`[Script] -> Executed (${script}):`, data) }
